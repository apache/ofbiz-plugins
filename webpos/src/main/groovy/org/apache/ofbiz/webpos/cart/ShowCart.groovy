...
if (shoppingCart) {
    ...
    for (i = 0; i < payments; i++) {
        ...
        if (paymentInfo.amount != null) {
            ...
            if (paymentInfo.paymentMethodTypeId) {
                switch (paymentInfo.paymentMethodTypeId) {
                    case 'CASH' -> context.cashAmount = new BigDecimal((context.cashAmount).add(amount))
...
context.paymentCash = from('PaymentMethodType').where('paymentMethodTypeId': 'CASH').cache().queryOne()
context.paymentCheck = from('PaymentMethodType').where('paymentMethodTypeId': 'PERSONAL_CHECK').cache().queryOne()
context.paymentGift = from('PaymentMethodType').where('paymentMethodTypeId': 'GIFT_CARD').cache().queryOne()
context.paymentCredit = from('PaymentMethodType').where('paymentMethodTypeId': 'CREDIT_CARD').cache().queryOne()
