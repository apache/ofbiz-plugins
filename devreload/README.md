...
| 3 | Inner and anonymous classes (e.g. `Foo$1.class`) weren't included when hot-reloading their outer class | Lambdas/anonymous classes kept running stale logic after a save | Scan the compiled output for every `Outer.class` and `Outer$*.class` file and reload them all together |
