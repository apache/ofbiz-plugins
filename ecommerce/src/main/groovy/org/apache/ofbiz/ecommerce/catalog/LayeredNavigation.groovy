...
searchCategoryId = parameters.searchCategoryId ?: context.productCategoryId
if (searchCategoryId) {
...
if (!parameters.clearSearch || parameters.clearSearch != 'N') {
...
if (subCategoryList) {
    ...
    subCategoryList.each { subCategory ->
        ...
        if (categoryCount > 0) {
            ...
            thisSubCategoryList << [productCategoryId: subCategory.productCategoryId,
                                    categoryName: subCategory.categoryName,
                                    count: categoryCount,
                                    categoryContentWrapper: subCategoryContentWrapper]
        }
    }
...
availablePriceRangeList = [[low: '0', high: '10'],
                           [low: '10', high: '20'],
                           [low: '20', high: '30'],
                           [low: '30', high: '40'],
                           [low: '40', high: '50'],
...
availablePriceRangeList.each { priceRange ->
    priceRangeConstraint = new ProductSearch.ListPriceRangeConstraint(
            new BigDecimal(priceRange.low), new BigDecimal(priceRange.high), UtilHttp.getCurrencyUom(request))
    if (searchConstraintList.contains(priceRangeConstraint)) {
        context.showPriceRange = false
    } else {
        priceRangeCount = ProductSearchSession.getCountForListPriceRange(
                new BigDecimal(priceRange.low), new BigDecimal(priceRange.high), session, delegator)
        if (priceRangeCount != 0) {
            priceRangeList << [low: priceRange.low, high: priceRange.high, count: priceRangeCount]
        }
    }
}
if (!priceRangeList) {
