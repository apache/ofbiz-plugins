      ...
      <#list productCompareList as product>
        <tr>
          ...
          <td>
            ...
            <a href="javascript:document.removeFromCompare${product_index}form.submit()" class="btn btn-link btn-sm">${uiLabelMap.CommonRemove}</a>
          </td>
        </tr>
      </#list>
    </table>
    <div>
      <a href="<@ofbizUrl>clearCompareList</@ofbizUrl>" class="btn btn-outline-secondary">${uiLabelMap.CommonClearAll}</a>
    </div>
    <div>
      <a href="javascript:popUp('<@ofbizUrl secure="${request.isSecure()?string}">compareProducts</@ofbizUrl>', 'compareProducts', '650', '750')" class="btn btn-outline-secondary">${uiLabelMap.ProductCompareProducts}</a>
    </div>
  <#else>
    <div>
