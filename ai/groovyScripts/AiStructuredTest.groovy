...
try {
    ...
    if (!result || !result.containsKey("word")) {
        Debug.logError("AI structured smoke test failed: response missing 'word' key. Got: " + result, "AiStructuredTest")
        return error("AI structured smoke test failed: missing 'word' key in response")
    }
    Debug.logInfo("AI structured smoke test response: " + result, "AiStructuredTest")
    return success("AI structured smoke test passed: " + result)
} catch (Exception e) {
    Debug.logError(e, "AI structured smoke test failed", "AiStructuredTest")
