...
partyAndSecurityGroupList = select('partyId', 'groupId').from('ScrumMemberUserLoginAndSecurityGroup')
        .where(securityGroupCond)
        .orderBy('partyId')
        .queryList()
oldProjectId = null
newProjectId = null
...
allSprints.each { sprint ->
    ...
    productAndRole = from('ProductAndRole')
            .where('roleTypeId', 'PRODUCT_OWNER_COMP', 'productId', sprint.productId)
            .queryList()
    companyId = ''
    companyName = ''
    ...
    if (oldProjectId != newProjectId) {
        ...
        if (partyAndSecurityGroupList) {
            ...
            if (groupId == 'SCRUM_PRODUCT_OWNER') {
                productAndRoleList = from('ProductRole')
                        .where('productId', sprint.productId, 'partyId', partyAndSecurityGroupList.first().partyId, 'thruDate', null)
                        .queryList()
                if (productAndRoleList) {
                    ismember = true
                }
            } else if (groupId == 'SCRUM_STAKEHOLDER') {
                ...
                productRoleList = from('ProductRole')
                        .where('partyId', scrumRolesPersonAndCompanyList[0].partyIdFrom, 'roleTypeId', 'PRODUCT_OWNER_COMP', 'thruDate', null)
                        .queryList()
                if (productRoleList) {
                    productRoleList.each { productRoleMap ->
                        stakeholderProduct = productRoleMap.productId
                        if (stakeholderProduct == sprint.productId) {
                            ismember = true
                        }
                    }
                }
                ...
                if (ismember == false) {
                    projectPartyAssignment = from('WorkEffortPartyAssignment')
                            .where('workEffortId', sprint.projectId, 'partyId', userLogin.partyId)
                            .queryList()
                ...
                if (ismember == false) {
                    ...
                    allSprintList.each {
                        sprintId = it.workEffortId
                        workEffortPartyAssignment = from('WorkEffortPartyAssignment')
                                .where('workEffortId', sprintId, 'partyId', userLogin.partyId)
                                .queryList()
        ...
        if (security.hasEntityPermission('SCRUM', '_ADMIN', session)
                || ((security.hasEntityPermission('SCRUM', '_ROLE_ADMIN', session)
                || security.hasEntityPermission('SCRUM', '_ROLE_VIEW', session)
                || security.hasEntityPermission('SCRUM_PROJECT', '_ROLE_ADMIN', session)
                || security.hasEntityPermission('SCRUM_PROJECT', '_ROLE_VIEW', session)
        ...
        if (countSprint < 4) {
            ...
            if (partyAndSecurityGroupList) {
                ...
                if (groupId == 'SCRUM_PRODUCT_OWNER') {
                    ...
                    if (ismember == false) {
                        allSprintList = []
                        allSprintList = from('WorkEffort')
                                .where('workEffortParentId', sprint.projectId)
                                .queryList()
                        allSprintList.each {
                            sprintId = it.workEffortId
                            workEffortPartyAssignment = from('WorkEffortPartyAssignment')
