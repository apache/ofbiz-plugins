...
try {
    ...
    categoryList = []
    Map condition = [custRequestTypeId: 'RF_PARENT_BACKLOG']
    if (productId) {
        condition.productId = productId
    }
    sprintList = from('CustRequestAndCustRequestItem').where(condition).queryList()
    if (sprintList) {
...
//subString function
String returnNameAsString(String input, int length) {
    if (input.length() > length) {
        ansValue = input.substring(0, Math.min(input.length(), length))
        return ansValue
    }
    return input
}
